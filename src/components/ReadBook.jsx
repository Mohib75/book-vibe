...
const ReadBook = () => {
	...
	const [displayBooksRead, setDisplayBooksRead] = useState([])

	useEffect(() => {
		if (filter === "all") {
			setDisplayBooksRead(booksRead)
		} else if (filter === "numberOfPages") {
			const sortedBooks = booksRead.slice().sort((a, b) => b.totalPages - a.totalPages)
			setDisplayBooksRead(sortedBooks)
		} else if (filter === "rating") {
			const sortedBooks = [...booksRead].sort((a, b) => b.rating - a.rating)
			setDisplayBooksRead(sortedBooks)
		} else if (filter === "publishedYear") {
			const sortedBooks = booksRead.slice().sort((a, b) => b.yearOfPublishing - a.yearOfPublishing)
			setDisplayBooksRead(sortedBooks)
		}
	}, [filter, booksRead])
	...
	useEffect(() => {
		...
		if (books.length > 0) {
			...
			setDisplayBooksRead(readBook)
		}
	}, [books])
	return (
		<>
			{displayBooksRead &&
				displayBooksRead.map((book, index) => (
					<div
						key={index}
